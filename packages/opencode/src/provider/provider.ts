...
export namespace Provider {
  ...
  async function getSDK(model: Model) {
    try {
          ...
          if (
            ...
            const stream = new ReadableStream({
              async start(controller) {
                let buffer = ""
                let potentialToolJson = ""
                let isBufferingJson = false
                let wasCoerced = false
                let lastJsonChunk: any = null
                let toolCallId = "call_" + Math.random().toString(36).slice(2, 11)

                const flushBufferingAsText = (reason: string) => {
                  if (!isBufferingJson) return
                  l.info("buffering_flush", { reason, preview: potentialToolJson.substring(0, 500) })
                  const textChunk = {
                    ...lastJsonChunk,
                    choices: [{
                      ...(lastJsonChunk?.choices?.[0] || { index: 0 }),
                      delta: { content: potentialToolJson }
                    }]
                  }
                ...
                try {
                  while (true) {
                    ...
                    if (done) {
                      flushBufferingAsText("stream_end")
                      if (buffer) {
                    ...
                    for (const line of lines) {
                      ...
                      if (trimmedLine.startsWith("data:")) {
                        ...
                        try {
                          ...
                          if (wasCoerced && json.choices?.[0]?.finish_reason) {
                          ...
                          if (content) {
                            if (!isBufferingJson && content.trim().startsWith("{")) {
                              l.info("buffering_start", { content })
                              isBufferingJson = true
                              potentialToolJson = content
                              continue
                            }
                            ...
                            if (isBufferingJson) {
                              ...
                              try {
                                  const parsed = JSON.parse(potentialToolJson)
                                  ...
                                  const name = target.name || target.function?.name || target.action || target.tool
                                  const args = target.arguments || target.parameters || target.function?.arguments || target.function?.parameters || target.args || target.input
                                  ...
                                  if (toolName && toolArgs) {
                                    l.info("coerced_tool_call", { tool: toolName })
                                    const toolChunk = {
                                      ...json,
                                      choices: [{
                                        ...json.choices[0],
                                    ...
                                    flushBufferingAsText("not_tool_call")
                                    continue
                                  }
                              } catch (e) {
                                // Keep buffering
      ...
      if (bundledFn) {
        log.info("using bundled provider", { providerID: model.providerID, pkg: bundledKey, optionKeys: Object.keys(options) })
